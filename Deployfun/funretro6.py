...
def show_qr():
    ...
    with col2:
        ...
        if st.button("✅ I’ve scanned it", use_container_width=True, type="primary"):
            st.session_state.step = 1

# --- Step 1: Theme Selection ---
def show_theme():
    st.markdown(center_text("🎭 Choose a Theme", "26px"), unsafe_allow_html=True)
    for label, tdata in themes.items():
        col1, col2, col3 = st.columns([1, 6, 1])
        with col2:
            if st.button(label, key=label, use_container_width=True):
                st.session_state.theme = label
                st.session_state.step = 2
            st.markdown(f"""
                <div style="height:6px; background-color:{tdata['color']}; margin-top:-10px; border-radius:8px;"></div>
